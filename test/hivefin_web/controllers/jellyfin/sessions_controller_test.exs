defmodule HivefinWeb.Jellyfin.SessionsControllerTest do
  ...
  # A client that compares the REST list against the socket push (or polls
  # one and subscribes to the other) must not see different freshness for
  # the same session — both emitters build from the same registry entry.
  test "GET /Sessions and the Sessions socket push agree on LastActivityDate", %{
    conn: conn,
    user: user,
    access_token: access_token
  } do
    ...
    socket_state = %{user_id: user.id, subscriptions: MapSet.new()}
    ...
    assert rest_session["LastActivityDate"] == socket_session["LastActivityDate"]
  end
